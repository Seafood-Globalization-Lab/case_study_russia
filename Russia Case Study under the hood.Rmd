...
Germany, The United States, and South Korea all decreased their imports from Russia after the annexation, although only Germany and the United States had imposed sanctions on Russia and faced the agricultural embargo. Kazakstan took the place of the USA in the top 10 importers after the annexation, but it was not enough to make up the difference from losses by the U.S.A.

```{r, echo=FALSE, include = FALSE}
kable(top_importers_all_years, caption = "Top 10 Importers of Russian Goods")
```

Due to Russia's agricultural embargo counter-sanction, which banned itself from importing agricultural goods from countries who imposed sanctions against Russia, Russia ramped up domestic good production, primarily in capture fisheries. In most cases, importing countries bought more capture products than aquaculture, with the exceptions being Great Britain, Moldova, Azerbaijan, and Vietnam. Great Britain stopped importing goods from Russia after 2014, and neither Moldova, Azerbaijan, nor Vietman imported capture seafood in either time period. Azerbaijan and Vietnam did not import any aquaculture products prior to 2014, but did after.
...
Some importing nations had more drastic changes in imports before and after the Annexation thatn other. The USA and Germany, for example, saw over a 50% decrease in imports from Russia, while other countries like the Netherlands and Nigeria experienced a 50% increase in Russian imports. 
...
Top seafood exporters to Russia include China, Iceland, Latvia, Morocco, Norway, and Ukraine, which all decreased total exports in the five years following the 2014 Annexation. Latvia, Iceland, Norway, and Ukraine stopped exporting to Russia completely. These four countries enacted sanctions against Russia in response to the Annexation, which meant that the Russian food embargo was placed on them in turn. In their place, Greenland, India, Mauritania, and Thailand rose to the top 10 exporters to Russia. These replacements also did not make up the difference from the lost tonnage of Iceland, Latvia, Norway, and Ukraine.

```{r, echo=FALSE, fig.cap= "Faceted by countries who imposed sanctions on Russia", message=FALSE}
...
Trade trends with Russia can be seen when the imports and exports are broken down by the top species, rather than country. It is evident that Russia stopped importing as many seafood goods due to its agricultural embargo. Instead, Russia increased domestic supply to become more agriculturally self-reliant and food secure, and exported much more than it did prior to the Annexation. Its primary method was capture fisheries. Russia exported more seafood than it imported by an order of magnitude.
...
```{r, echo = FALSE, message=FALSE, fig.height= 6, fig.cap = "Filtered by Change greater than 10,000 tons to show highest Exporting Countries to Russia"}
...
Countries that imposed sanctions on Russia, for the most part, still continued or increased their Russian seafood imports. The exceptions were Germany, and the USA. South Korea's imports also decreased, but it would go on to adopt Western sanctions against Russia in the Russian-Ukrainian War in 2022. Still, with variable degrees of import changes indeterminate of sanction status, there is no clear trend to predict import changes.
...
# Getting all importers limit by threshold
importers <- artis_russia_2009_2019 %>%
    filter(year %in% 2010:2014) %>%
  group_by(importer_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  ungroup() %>%
...
# Change all producers, exporters, and importers names based on limit threshold
artis_thresholded <- artis_russia_2009_2019 %>%
    filter(year %in% 2010:2014) %>%
  mutate(
    exporter_iso3c = case_when(
      exporter_iso3c %in% unique(c(producers$source_country_iso3c, exporters$exporter_iso3c, importers$importer_iso3c)) ~ exporter_iso3c,
      TRUE ~ "Other"
    ),
    importer_iso3c = case_when(
      importer_iso3c %in% unique(c(producers$source_country_iso3c, exporters$exporter_iso3c, importers$importer_iso3c)) ~ importer_iso3c,
      TRUE ~ "Other"
    ),
    source_country_iso3c = case_when(
      source_country_iso3c %in% unique(c(producers$source_country_iso3c, exporters$exporter_iso3c, importers$importer_iso3c)) ~ source_country_iso3c,
      TRUE ~ "Other"
    )
  ) %>%
...
# Summarize trade flows from producers to exporters
source_exporter <- artis_thresholded %>%

  group_by(source_country_iso3c, exporter_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  mutate(source_country_iso3c = paste(source_country_iso3c, "_source", sep=""),
         exporter_iso3c = paste(exporter_iso3c, "_exp", sep="")) %>%
  rename(source = source_country_iso3c,
         target = exporter_iso3c)

# Summarize trade flows from exporters to importers
exporter_importer <- artis_thresholded %>%

  group_by(exporter_iso3c, importer_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  mutate(exporter_iso3c = paste(exporter_iso3c, "_exp", sep=""),
         importer_iso3c = paste(importer_iso3c, "_imp", sep="")) %>%
  rename(source = exporter_iso3c,
         target = importer_iso3c)

# Overall trade flow dataframes from producers -> exporters -> importers
tmp <- source_exporter %>%
  bind_rows(exporter_importer)

# Creating node dataframe from country names to node id
nodes <- data.frame(
  # Unique list of trading entity names
  name = unique(c(tmp$source, tmp$target))
  ) %>%
  # labels for sankey diagram only include country iso3 code
  mutate(label = gsub("_.{3,6}", "", name))

# Map source and target names to row in the nodes dataframe, 0-indexed
tmp <- tmp %>%
  mutate(source_id = match(source, nodes$name) - 1,
         target_id = match(target, nodes$name) - 1)

# Create Sankey Diagram
sankeyNetwork(Links=tmp, Nodes = nodes, Source="source_id", Target="target_id",
              Value="live_weight_t", NodeID="label")


# Showing all trade flows from producers to exporters to importers, 2015-2019
...
producers <- artis_russia_2009_2019 %>%
  filter(year %in% 2010:2014) %>%
  group_by(source_country_iso3c, exporter_iso3c == "DNK") %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  ungroup() %>%
  mutate(total = sum(live_weight_t)) %>%
  mutate(prop = live_weight_t / total) %>%
...
# Getting all importers limit by threshold
importers <- artis_russia_2009_2019 %>%
    filter(year %in% 2010:2014, exporter_iso3c == "DNK") %>%
  group_by(importer_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  ungroup() %>%
  mutate(total = sum(live_weight_t)) %>%
...
# Getting all importers limit by threshold
importers <- artis_russia_2009_2019 %>%
    filter(year %in% 2015:2019, exporter_iso3c == "DNK") %>%
  group_by(importer_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  ungroup() %>%
  mutate(total = sum(live_weight_t)) %>%
...
A closer look at the trade through Denmark shows the changes in source countries and importers from 2010-2014 and 2015-2019. After the Annexation, source countries Canada, Iceland, Mauritania, and Norway were replaced with Greenland, India, and Morocco. Mauritania was the only source before the Annexation that did not impose sanctions on Russia. Denmark continued receiving seafood from Chile, China, Russia, Vietnam, and in its own waters in both time periods. Denmark's continued trade with Russia may be a contributing factor as to why it stopped receiving seafood from Canada, Iceland, and Norway- countries that had imposed sanctions of Russia, but it may also be due to the .01% threshold placed on the calculations, putting those countries' decreased trade into the nearly quadrupled "Other" category.
The real mystery is why Denmark's exports to Russia nearly tripled (373,212 tons -> 977,581 tons), when Russia's agricultural embargo also should have applied to Denmark. A total of 381,436 tons of seafood products flowed through Denmark before the Annexation, and 985,261 tons flowed after, so its trade with Russia remained proportional, even though it should have been 0.
...
# Directory Structure
# datadir <- "data"
# filename <- "artis_russia_case.csv" # ARTIS Russia Case Study
# 
# artis <- read.csv(file.path(datadir, filename))

################################################################################
# Example 1
# Showing trade relationships where Russia is the main producer or intermediate exporter

# Threshold for creating an Other importer group
threshold <- 0.01

# Create a links dataframe
russia_prod_exp <- artis_russia_2009_2019 %>%
  # Filtering for all trade relationships where Russia is producer OR exporter
  filter(source_country_iso3c == "RUS" |
           exporter_iso3c == "RUS") %>%
  group_by(importer_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  ungroup() %>%
  # Getting proportion of total trade for each direct and indirect importer from Russia 
  mutate(total = sum(live_weight_t)) %>%
  mutate(prop = (live_weight_t / total)) %>%
  # Using proportion to create an Other importer group
  mutate(importer_iso3c = case_when(
    prop < threshold ~ "Other",
    TRUE ~ importer_iso3c)) %>%
  # Re-summarizing trade flows using Other group
  group_by(importer_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t))

# Creating a Node dataframe
nodes <- data.frame(
  country = c("RUS", unique(russia_prod_exp$importer_iso3c))
)

nodes <- nodes %>%
  mutate(node_id = seq(from=0, to=nrow(russia_prod_exp)))

# Adding exporter column and node ids
russia_prod_exp <- russia_prod_exp %>%
  left_join(nodes, by=c("importer_iso3c"="country")) %>%
  mutate(exporter_nodes = 0) %>%
  rename(importer_nodes = node_id) %>%
  select(exporter_nodes, importer_nodes, live_weight_t)

# Creating Sankey Diagram
sankeyNetwork(
  Links=russia_prod_exp, Nodes=nodes %>% select(country), Source="exporter_nodes", Target="importer_nodes",
  Value="live_weight_t", NodeID="country"
)

################################################################################
# Example 2
# Trade relationships where Russia is the importer

# Threshold for creating an Other exporter group
threshold <- 0.01

russia_imp <- artis_russia_2009_2019 %>%
  # Filter trade relationships where the importer is Russia
  filter(importer_iso3c == "RUS") %>%
  group_by(exporter_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t)) %>%
  ungroup() %>%
  # Create trade flow proportions and filter to create Other importer group
  mutate(total = sum(live_weight_t)) %>%
  mutate(prop = live_weight_t / total) %>%
  mutate(exporter_iso3c = case_when(
    prop < threshold ~ "Other",
    TRUE ~ exporter_iso3c)) %>%
  # Re-summarize with Other exporter group
  group_by(exporter_iso3c) %>%
  summarize(live_weight_t = sum(live_weight_t))

# Create node dataframe relating country names to node ID's
nodes <- data.frame(
  country = c("RUS", unique(russia_imp$exporter_iso3c))
)

nodes <- nodes %>%
  mutate(node_id = seq(from=0, to=nrow(russia_imp)))

russia_imp <- russia_imp %>%
  left_join(nodes, by=c("exporter_iso3c"="country")) %>%
  mutate(importer_nodes = 0) %>%
  rename(exporter_nodes = node_id) %>%
  select(exporter_nodes, importer_nodes, live_weight_t)

sankeyNetwork(
  Links=russia_imp, Nodes=nodes %>% select(country), Source="exporter_nodes", Target="importer_nodes",
  Value="live_weight_t", NodeID="country"
)
################################################################################
# Example 3
# Showing all trade flows from producers to exporters to importers
...
# library(devtools)
# # devtools::install_github("Seafood-Globalization-Lab/exploreARTIS")
# 
# library(exploreARTIS)
# 
# plot_sankey(artis_russia_2009_2019, producers = "RUS")
# plot_species_line(artis_russia_2009_2019 %>%
#                     rename(sciname= sci_name), producers = "RUS")
# plot_species_stacked(artis_russia_2009_2019%>%
#                     rename(sciname= sci_name), producers = "RUS")
# plot_partner_line(artis_russia_2009_2019 %>%
#                     rename(sciname = sci_name)
#                     , producers = "RUS", 
#                   trade_flow = "import")
# plot_partner_stacked(artis_russia_2009_2019 %>%
#                     rename(sciname = sci_name)
#                     , producers = "RUS", 
#                   trade_flow = "import")
# plot_chord(artis_russia_2009_2019 %>%
#                     rename(sciname = sci_name), 
#            focal_country = "RUS",
#            species = "salmo salar",
#            years = 2009,
