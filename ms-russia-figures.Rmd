...
artis <- artis %>%
  # Add details on sanctions following annexation of Crimea
...
consumption <- consumption %>%
  ...
  # Add details on sanctions following annexation of Crimea
    mutate(sanctions_towards_russia = case_when(
      consumer_iso3c %in% c("USA", "AUT", "BEL", "BGR", "HRV", "CYP", "CZE", "DNK", "EST", "FIN", "FRA", "DEU", "GRC", "HUN", "IRL", "ITA", "LVA", "LTU", "LUX", "MLT", 
                            "NLD", # FIXIT: Need to find out what is happening with the Netherlands and others directly importing
                            "POL", "PRT", "ROM", "SVK", "SVN", "ESP", "SWE", "GBR", "JPN", "ALB", "ISL", "AUS", "MNT", "UKR", "CAN", "NOR", "CHE", "MDA")  ~ "Imposed sanctions",
      TRUE ~ "Did not impose sanctions"
    )) %>%
      mutate(exporter_sanctions_towards_russia = case_when(
      exporter_iso3c %in% c("USA", "AUT", "BEL", "BGR", "HRV", "CYP", "CZE", "DNK", "EST", "FIN", "FRA", "DEU", "GRC", "HUN", "IRL", "ITA", "LVA", "LTU", "LUX", "MLT", "NLD", "POL", "PRT", "ROM", "SVK", "SVN", "ESP", "SWE", "GBR", "JPN", "ALB", "ISL", "AUS", "MNT", "UKR", "CAN", "NOR", "CHE", "MDA")  ~ "Imposed sanctions",
      TRUE ~ "Did not impose sanctions")) %>%
    mutate(source_sanctions_towards_russia = case_when(
      source_country_iso3c %in% c("USA", "AUT", "BEL", "BGR", "HRV", "CYP", "CZE", "DNK", "EST", "FIN", "FRA", "DEU", "GRC", "HUN", "IRL", "ITA", "LVA", "LTU", "LUX", "MLT", "NLD", "POL", "PRT", "ROM", "SVK", "SVN", "ESP", "SWE", "GBR", "JPN", "ALB", "ISL", "AUS", "MNT", "UKR", "CAN", "NOR", "CHE", "MDA")  ~ "Imposed sanctions",
      TRUE ~ "Did not impose sanctions")) %>% 
    left_join(sciname_metadata %>% 
              select(sciname, common_name), 
            by = "sciname") %>%
...
consumption %>%
  ...
  labs(x = "", y = "Consumption of Russian seafood (mil t, live weight)", fill = "") +
...
consumption %>%
  ...
  labs(x = "", y = "Consumption of Russian seafood (mil t, live weight)", color = "") +
  facet_wrap(~sanctions_towards_russia) +
  theme_minimal()
...
consumption_change <- consumption %>%
  filter(sanctions_towards_russia == "Imposed sanctions", 
         source_country_iso3c == "RUS") %>%
  group_by(year) %>%
...
consumption_change <- consumption %>%
  filter(sanctions_towards_russia == "Did not impose sanctions", 
         source_country_iso3c == "RUS") %>%
  group_by(year) %>%
  summarise(consumption_live_t = sum(consumption_live_t, na.rm = TRUE))
...
consumption_change <- consumption %>%
  filter(source_sanctions_towards_russia == "Imposed sanctions", 
         consumer_iso3c == "RUS") %>%
  group_by(year) %>%
  summarise(consumption_live_t = sum(consumption_live_t, na.rm = TRUE))
...
consumption %>%
  ...
  group_by(year, consumer_iso3c, path) %>%
  summarise(consumption_live_t = sum(consumption_live_t))
...
p_joined <- ggarrange(p1, p2, p3, ncol = 1)
...
chn_to_usa %>% 
  filter(year == 2020, 
         sciname %in% c("gadus chalcogrammus", "gadus morhua", "oncorhynchus gorbuscha"))
